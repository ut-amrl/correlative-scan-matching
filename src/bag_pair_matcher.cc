...
DEFINE_string(bag_file, "",
              "Bag file from which to read scans. Only works if "
              "scan_match_topic isn't provided.");
DEFINE_string(lidar_topic, "/Cobot/Laser",
              "topic within bag file which to read scans. Only works if "
              "scan_match_topic isn't provided.");
...
DEFINE_double(base_timestamp, 0.0,
              "The timestamp in the bag file for the base scan.");
DEFINE_double(match_timestamp, 0.0,
...
void scan_match_bag_file(string bag_path, string lidar_topic,
                         double base_timestamp, double match_timestamp,
                         bool truncate_scan_angles, bool calc_uncertainty) {
  ...
  std::pair<double, std::pair<Vector2f, float>> matchResult =
      matcher.GetTransformation(baseCloud, matchCloud);
  fflush(stdout);
  double prob = matchResult.first;
  std::pair<Vector2f, float> trans = matchResult.second;
  printf(
      "recovered relative translation: (%f, %f), rotation: %f with score %f\n",
      trans.first.x(), trans.first.y(), trans.second, prob);
  std::pair<double, std::pair<Vector2f, float>> baseResult =
      matcher.GetTransformation(matchCloud, baseCloud);
  printf(
      "recovered relative translation: (%f, %f), rotation: %f with score %f\n",
      baseResult.second.first.x(), baseResult.second.first.y(),
      baseResult.second.second, prob);

  printf("Visualizing results...\n");
  Affine2f transform =
      Translation2f(trans.first) * Rotation2Df(trans.second).toRotationMatrix();
  ...
  Affine2f transform_match_affine =
      Translation2f(baseResult.second.first) *
      Rotation2Df(baseResult.second.second).toRotationMatrix();
  ...
  cimg_library::CImg<double> match_image_transformed =
      matcher.GetLookupTableHighRes(matchTransformed).GetDebugImage();
  cimg_library::CImg<double> base_image = high_res_lookup.GetDebugImage();
  cimg_library::CImg<double> base_image_transformed =
      matcher.GetLookupTableHighRes(baseTransformed).GetDebugImage();
  cimg_library::CImg<double> base_transformed_image(
      base_image.width(), base_image.height(), 1, 3, 0);
  cimg_library::CImg<double> match_transformed_image(
      base_image.width(), base_image.height(), 1, 3, 0);
  ...
  if (calc_uncertainty) {
    ...
    Matrix2f uncertainty =
        matcher.GetUncertaintyMatrix(baseCloud, matchCloud, trans.second);
    ...
    CImg<unsigned char> uncertainty_img(base_image_transformed.width(),
                                        base_image_transformed.height(), 1, 3,
                                        0);
    const unsigned char color[] = {0, 0, 255};
    uncertainty_img.draw_ellipse(
        uncertainty_img.width() / 2, uncertainty_img.height() / 2,
        eigenvalues[0] * 50, eigenvalues[1] * 50,
        atan(eigenvectors.col(0)[1] / eigenvectors.col(0)[0]), color, 1.0);
    std::cout << "Condition #: "
              << eigenvalues.maxCoeff() / eigenvalues.minCoeff() << std::endl;
...
int main(int argc, char** argv) {
  ...
  if (FLAGS_bag_file.compare("") != 0 && FLAGS_lidar_topic.compare("") != 0 &&
      FLAGS_base_timestamp != 0.0 && FLAGS_match_timestamp != 0.0) {
    scan_match_bag_file(FLAGS_bag_file, FLAGS_lidar_topic, FLAGS_base_timestamp,
                        FLAGS_match_timestamp, FLAGS_truncate_scan_angles,
                        FLAGS_calc_uncertainty);
  } else {
