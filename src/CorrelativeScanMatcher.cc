...
vector<pair<double, pair<Vector2f, float>>> MemoizeLowRes(
...
CorrelativeScanMatcher::GetProbAndTransformation(
    ...
    for (double y_trans = y_min + EPSILON; y_trans < y_max;
      ...
      if (probability > current_most_likely_prob) {
        current_most_likely_trans =
            pair<Eigen::Vector2f, float>(Vector2f(x_trans, y_trans), rotation);
...
CorrelativeScanMatcher::GetTransformation(const vector<Vector2f>& pointcloud_a,
  ...
  vector<pair<double, pair<Vector2f, float>>> low_res_costs =
      MemoizeLowRes(pointcloud_a, pointcloud_b_cost_low_res, low_res_,
                    trans_range_, rotation_min, rotation_max);
#if DEBUG
  ...
  while (current_probability >= best_probability) {
    ...
    std::vector<pair<double, pair<Vector2f, float>>>::iterator max =
        std::max_element(low_res_costs.begin(), low_res_costs.end(),
                         LessThanTransProb);
...
#pragma omp parallel for
  for (size_t i = 0; i < comparisonClouds.size(); i++) {
    ...
    std::pair<double, std::pair<Vector2f, float>> result =
        GetTransformation(targetCloud, comparisonCloud);
